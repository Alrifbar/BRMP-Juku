...
<head>
    ...
    <title>Web Settings - Admin Panel</title>
...
<body x-data="{ theme: '{{ Session::get('theme', 'light') }}' }" x-init="if(theme==='dark'){ document.documentElement.classList.add('dark'); }" class="bg-[#EEF2FF] dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-100">
    @include('layouts.admin_navbar')

    <main class="max-container py-8 px-4 sm:px-6 lg:px-8 pb-24 md:pb-8 text-sm md:text-base">
        <!-- Header -->
        <div class="mb-8 fade-in">
            <div class="executive-card bg-white dark:bg-gray-800 rounded-2xl p-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Web Settings</h2>
                        <p class="text-gray-600 dark:text-gray-300">Kelola pengaturan web admin</p>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="executive-card bg-white dark:bg-gray-800 rounded-2xl p-6 fade-in">
                ...
                <form action="{{ route('admin.settings.preferences') }}" method="POST" class="space-y-3">
                    @csrf
                    <label class="flex items-center justify-between bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3">
                        <span class="text-gray-800 dark:text-gray-200">Jurnal baru (agregasi)</span>
            ...
            <div class="executive-card bg-white dark:bg-gray-800 rounded-2xl p-6 fade-in">
                ...
                <form action="{{ route('admin.settings.theme') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1">Theme</label>
                        <div class="flex items-center space-x-3">
                            <label class="inline-flex items-center space-x-2 cursor-pointer">
                                <input type="radio" name="theme" value="light" x-model="theme" @change="document.documentElement.classList.remove('dark')" {{ (Session::get('theme','light') === 'light') ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                                <span class="text-gray-800 dark:text-gray-200">Light</span>
                            </label>
                            <label class="inline-flex items-center space-x-2 cursor-pointer">
                                <input type="radio" name="theme" value="dark" x-model="theme" @change="document.documentElement.classList.add('dark')" {{ (Session::get('theme','light') === 'dark') ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <select name="default_page" class="border border-gray-300 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 w-full">
                            ...
                            <option value="dashboard" {{ (Session::get('default_page','')==='dashboard') ? 'selected' : '' }}>Dashboard</option>
                            <option value="notifications" {{ (Session::get('default_page','')==='notifications') ? 'selected' : '' }}>Notifikasi</option>
                        </select>
                    </div>
